...
def download_report():
    if request.method == 'POST':
        ...
        file_data = db.collection('files').document(site_id).get()
        if file_data.exists:
            ...
            if file_url:
                ...
                return "File do no exist in our system"
        else:
            return "File do no exist in our system"
...
def integrate():
    if request.method == "POST":
        ...
        try:
            ...
            dataList = []
            datas = db.collection('circle') \
                       .where('status', '==', circle_status) \
                       .where('circlename', '==', circle_name) \
                       .where('completedate', '>=', from_date) \
                       .where('completedate', '<=', to_date) \
                       .stream()
            for data in datas:
                dataList.append(data.to_dict())
            print("Fetched")
            ...
            print(circle_status)
            print(dataList)
            return render_template('integrate.html', circle_name=circle_name, from_date=from_date, data=dataList)

        except Exception as e:
            # Handle any errors that occur during data retrieval

            return "Error fetching data from Firestore. Please try again later."
...
def logdetails():
    ...
    data = []
    site_id = 'SiteId121'
    site_doc = db.collection('sites').document('SiteId121')
    sectors = ['sector1', 'sector2', 'sector3']
    for sector in sectors:
        site_doc_ref = db.collection('sites').document(site_id)
        log_details_ref = site_doc.collection(sector).document('LogDetails')
        log_details_data = log_details_ref.get().to_dict()
        if log_details_data:
            sector_name = sector
            # Create a dictionary for each item with Technology and Status
            technology = log_details_data.get('Technology', '')
    ...
    return render_template('logdetails.html', data=data)
...
def save_images(image_keys, sec, siteCode):
    ...
    excel_blob = bucket.blob('ExcelFiles/' + siteCode + '/images.xlsx')
    ...
    zip_blob = bucket.blob('zipFiles/'+siteCode+'/Predata_RAR.zip')
...
def postupload_images_1():
    ...
    if azimuth_file:
        ...
        if texts:
            extracted_text = texts[0].description
            doc_ref = db.collection('PostData').document('TYDR1').collection('sector1').document('data')
            doc = doc_ref.get()
            if doc.exists:
                data = doc.to_dict()
...
def postupload_images_2():
    ...
    if azimuth_file:
        ...
        if texts:
            extracted_text = texts[0].description
            doc_ref = db.collection('PostData').document('TYDR1').collection('sector2').document('data')
            doc = doc_ref.get()
            if doc.exists:
                data = doc.to_dict()
                expected_text = data.get('azimuth', '')
...
def postupload_images_3():
    ...
    if azimuth_file:
        ...
        if texts:
            extracted_text = texts[0].description
            doc_ref = db.collection('PostData').document('TYDR1').collection('sector3').document('data')
            doc = doc_ref.get()
            if doc.exists:
                data = doc.to_dict()
                expected_text = data.get('azimuth', '')
